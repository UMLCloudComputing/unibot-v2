...
async def test_chunk_url_behavior_success():
  async with mock_docling() as base_url:
    ...
    # Setup global vars specific for test
    async with aiohttp.ClientSession() as session:
      await chunk_url(session, test_url, task_registry, chunk_queue, base_url, mock_pbar)
...
async def test_chunk_url_high_volume_throughput():
  async with mock_docling() as base_url:
    ...
    async with aiohttp.ClientSession() as session:
      tasks = [chunk_url(session, f"http://url-{i}.com", task_registry, chunk_queue, base_url, mock_pbar) for i in range(NUM_URLS)]
