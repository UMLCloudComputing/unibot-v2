    ...
    async with mock_docling() as docling_base_url:
        async with aiohttp.ClientSession() as session:
            for count in counts:
                ...
                tasks = [
                    chunk_url(session, f"http://url-{i}.com", {}, chunk_queue, docling_base_url)
        ...
        async with aiohttp.ClientSession() as session:
            for count in counts:
                ...
                async def consume_generator(url):
                    async for chunk in chunk_url_generator(session, url, {}, docling_base_url):
...
def plot_results(data_vanilla, data_gen, filename):
    ...
    plt.plot(data_vanilla[0], data_vanilla[1], marker='o', linestyle='-', 
             color='#2980b9', label='Internal Queue (Vanilla)')
    plt.plot(data_gen[0], data_gen[1], marker='s', linestyle='--', 
             color='#e67e22', label='Generator (External Put)')
    ...
    plt.annotate(f"{data_vanilla[1][-1]:.2f}s", (data_vanilla[0][-1], data_vanilla[1][-1]), textcoords="offset points", xytext=(0,10), ha='center')
    plt.annotate(f"{data_gen[1][-1]:.2f}s", (data_gen[0][-1], data_gen[1][-1]), textcoords="offset points", xytext=(0,10), ha='center')
