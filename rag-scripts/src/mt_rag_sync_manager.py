...
def write_action_file(filepath, urls):
    ...
    try:
        with open(filepath, "w") as f:
            for url in urls:
                f.write(f"{url}\n")
            logging.info(f"FILE_EXPORT: Created '{filepath}' with {len(urls)} entries.")
    except Exception as e:
...
def remove_from_db(urls, conn: sqlite3.Connection):
    ...
    if not urls:
        return None
    conn.executemany("DELETE FROM url_tracker WHERE url = ?", [(url,) for url in urls])
...
def main():
    ...
    try:
        ...
        # Exit with code 1, critical error
        sys.exit(1)
    ...
    with ThreadPoolExecutor(max_workers=100) as pool:
        futures = [
            pool.submit(hasher, url)
            for url in tqdm(current_urls, desc="Submitting URLs", unit="url")
        ]

        for future in tqdm(
            as_completed(futures),
            total=len(futures),
            desc="Processing and Hashing URLs",
            unit="url",
        ):
            ...
            if action == "add":
                ...
                to_add.append(url)
                cursor.execute(
                    "INSERT OR REPLACE INTO url_tracker VALUES (?, ?, ?)",
                    (url, digest, datetime.now().isoformat()),
                )
            elif action == "refresh":
                ...
                to_add.append(url)
                cursor.execute(
                    "UPDATE url_tracker SET content_hash = ?, last_updated = ? WHERE url = ?",
                    (digest, datetime.now().isoformat(), url),
                )
            else:
    ...
    print(
        f"Check {OP_FILES_PATH}to_add.txt and {OP_FILES_PATH}to_delete.txt for details"
    )
