...
def main():
    ...
    for _ in range(NUM_CHUNKERS):
        chunking_pool.submit(chunk_url_sync, DOCLING_BASE_URL, url_queue, chunk_queue, chunk_pbar)

    # Start embedding threads
    for _ in range(NUM_EMBEDDERS):
        embedding_pool.submit(embedder_sync, OLLAMA_BASE_URL, chunk_queue, embedding_queue, BATCH_SIZE, embedding_pbar)

    # Start inserter threads
    for _ in range(NUM_INSERTERS):
        inserter_pool.submit(milvus_worker, embedding_queue, MILVUS_HOST, MILVUS_PORT, COLLECTION_NAME, embedder_stop_event, milvus_pbar)
